...
{
	//载入原图
	g_srcImage = imread("F:\\opencv\\6_imgproc\\images\\1.jpg", 1);
	if (!g_srcImage.data)
	{
		printf("读取图片Image0错误！\n");
		return false;
	...
	while (1)
	{
		...
		case '1':
			if (g_bIsColor)//若原来为彩色，转为灰度图，并且将掩模mask所有元素设为0
			{
			...
			else//若原来为灰度图，便将原来的彩图image0再次复制给image，并且将掩模mask所有元素设置为0
			{
				cout << "键盘“1”被按下，切换色彩/灰度模式，当前操作为将【彩色模式】切换为【灰度模式】\n";
